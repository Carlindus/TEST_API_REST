<template>
  <v-btn @click="generateRandomMessage" color="darken-1" elevation="4">
    Générer un message d'excuse
  </v-btn>
</template>
...
export default {
  ...
  created() {
    /*
    this.$axios
      .get('http://localhost:8087/symbolit/rest/excuses/list', {
        headers: {
          Accept: 'application/json',
        },
      })
      .then((res) => {
        this.fetchDataList(res.data);
      });
      */
    // "Tricherie" pour l'exercice après de longues recherches infructueuses
    this.fetchDataList(this.getDataList());

    // Emet le premier message
    if (this.excuseId != -1) {
      this.getSpecificMessage(this.excuseId);
    } else {
  ...
  methods: {
    fetchDataList(datalist) {
      this.devExcusesList = datalist;
    },
    getSpecificMessage(id) {
      const index = this.devExcusesList.findIndex((i) => i.http_code === id);
      if (index > -1) {
        this.displayMessage(this.devExcusesList[index].message);
      } else {
        this.generateRandomMessage();
      }
    },
    ...
    generateRandomMessage() {
      if (this.devExcusesList.length > 1) {
        let newMessage;
        do {
          newMessage = this.getRandomMessage();
        } while (newMessage == this.messageDisplayed);
    ...
    getRandomExcuse() {
      let randomExcuse;
      if (this.devExcusesList.length > 0) {
        const randomNumber = Math.floor(
          Math.random() * (this.devExcusesList.length - 1)
        );
        randomExcuse = this.devExcusesList[randomNumber];
      }
      return randomExcuse;
    },
    ...
    getRandomMessage() {
      let randomMessage;
      const randomExcuse = this.getRandomExcuse();
      if (randomExcuse != null) {
        randomMessage = randomExcuse['message'];
      }
    ...
    displayMessage(message) {
      ...
      this.$emit('generateMessage', this.messageDisplayed);
    },

    /**
     * Problème de CORS origin donc Adaptation pour le TEST
     */
    getDataList() {
      return [
        {
        ...
        },
        {
          http_code: 732,
          tag: 'Fucking',
          message: ' Fucking UnicðŸ’©de (il faut utilisé UTF8MB4)',
        },
        {
          http_code: 733,
          tag: 'Fucking',
        ...
        },
        {
          http_code: 785,
          tag: "Somebody Else's Problem",
          message: ' TPS Cover Sheet not attached',
        },
        {
          http_code: 786,
          tag: "Somebody Else's Problem",
        ...
        },
        {
          ...
          message: ' The Internet shut down due to copyright restrictions',
        },
        {
          http_code: 792,
          tag: 'Internet crashed',
          message: ' Climate change driven catastrophic weather event',
        },
        {
          http_code: 793,
          tag: 'Internet crashed',
        ...
        },
        {
          http_code: 797,
          tag: 'Internet crashed',
          message: ' This is the last page of the Internet. Go back',
        },
        {
          http_code: 798,
          tag: 'Internet crashed',
